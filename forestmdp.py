...
P, R = h.example.forest(S=5000, r1=4, r2=2, p=z, is_sparse=False)
print("--------------------")
print("P: ")
print(P)
print("--------------------")
...
P, R = h.example.forest(S=5000, r1=4, r2=2, p=z, is_sparse=False)
vi = h.mdp.ValueIteration(P, R, 0.95)
vi.setVerbose()
vi.run()
print(vi.policy)
print("---------POLICY ITERATION-----------")
P, R = h.example.forest(S=5000, r1=4, r2=2, p=z, is_sparse=False)
vi = h.mdp.PolicyIteration(P, R, 0.95)
vi.setVerbose()
vi.run()
print(vi.policy)
print("--------Q LEARNING-----------")
P, R = h.example.forest(S=5000, r1=4, r2=2, p=z, is_sparse=False)
vi = h.mdp.QLearning(P, R, 0.95)
vi.setVerbose()
vi.run()
print(vi.policy)
...
gamma_arr = [0] * 10
for i in range(0,10):
    pi = h.mdp.PolicyIteration(P, R, (i+0.5)/10)
    pi.run()
    gamma_arr[i]=(i+0.5)/10
    value_f[i] = _np.mean(pi.V)
    policy[i] = pi.policy
    iters[i] = pi.iter
...
gamma_arr = [0] * 10
for i in range(0,10):
    pi = h.mdp.ValueIteration(P, R, (i+0.5)/10)
    pi.run()
    gamma_arr[i]=(i+0.5)/10
    value_f[i] = _np.mean(pi.V)
    policy[i] = pi.policy
...
for epsilon in [0.05,0.15,0.25,0.5,0.75,0.95]:
    st = time.time()
    pi = QLearning(P,R,0.95, n_iter=10000)
    end = time.time()
    pi.run(epsilon)
    rew_array.append(pi.reward_array)
    value_f.append(_np.mean(pi.V))
